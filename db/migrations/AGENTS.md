...
- **SQL puro**, par obrigatório:
  `NNNN_identidade_{subdominio}_{descricao}.up.sql` +
  `...down.sql`, numeração **sequencial sem buracos** (o `migrate validate`
  reprova buraco).
...
- **`CREATE INDEX CONCURRENTLY` isolado** num arquivo próprio, sem
  transação, **um comando por arquivo** — é a única exceção ao "transacional
  por padrão".
